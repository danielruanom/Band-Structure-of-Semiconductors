...
for cond in {'AlP'}:#'C','Si','Ge','GaAs','AlP','GaP','AlAs','GaSb','Sn','SiC','AlSb','InP','InAs','InSb','ZnSe','ZnTe'}: # 'GaAs',
    ...
    E_eigenvectors_valencia_2=np.zeros((n,E_dim), dtype=complex)
    E_eigenvectors_valencia_3=np.zeros((n,E_dim), dtype=complex)
    E_eigenvectors_valencia_4=np.zeros((n,E_dim), dtype=complex)
    ...
    for j in range(n): #hay que modificar el for para los rangos que miramos
        ...
        E_eigenvectors_valencia[j,:] = np.abs(eigenvectors[:,3])
        E_eigenvectors_valencia_2[j,:] = np.abs(eigenvectors[:,2])
        E_eigenvectors_valencia_3[j,:] = np.abs(eigenvectors[:,1])
        E_eigenvectors_valencia_4[j,:] = np.abs(eigenvectors[:,0])
    ...
    plt.plot(k_norm, E_eigenvalues[:,0].real,color='k', linewidth=1.5, label='E1')  # Take real part
    plt.plot(k_norm, E_eigenvalues[:,1].real,color='k', linewidth=1.5, label='E2')  # Take real part
    plt.plot(k_norm, E_eigenvalues[:,2].real,color='k', linewidth=1.5, label='E3')  # Take real part
    plt.plot(k_norm, E_eigenvalues[:,3].real,color='k', linewidth=1.5, label='E4')  # Take real part
    plt.plot(k_norm, E_eigenvalues[:,4].real,color='k', linewidth=1.5, label='E5')  # Take real part
    plt.plot(k_norm, E_eigenvalues[:,5].real,color='k', linewidth=1.5, label='E6')  # Take real part
    plt.plot(k_norm, E_eigenvalues[:,6].real,color='k', linewidth=1.5, label='E7')  # Take real part
    plt.plot(k_norm, E_eigenvalues[:,7].real,color='k', linewidth=1.5, label='E8')  # Take real part
    plt.plot(k_norm, E_eigenvalues[:,8].real,color='k', linewidth=1.5, label='E9')  # Take real part
    plt.plot(k_norm, E_eigenvalues[:,9].real,color='k', linewidth=1.5, label='E10')  # Take real part
    ...
    # Add vertical lines
    plt.axvline(x=k_norm[m_LG-1], color='k', linestyle='--', linewidth=1.5)   # L -> Γ
    plt.axvline(x=k_norm[m_LG+m_GX-1], color='k', linestyle='--', linewidth=1.5) # Γ -> X
    plt.axvline(x=k_norm[m_LG+m_GX+m_XK-1], color='k', linestyle='--', linewidth=1.5) # X -> K

    # Add horizontal lines at the energy values where the min and max energies occur
    plt.axhline(y=min_conducion_energy, color='k', linestyle='--', linewidth=1.5)
    plt.axhline(y=max_valencia_energy, color='k', linestyle='--', linewidth=1.5)
    ...
    plt.text(-1.0, -17.0, 'L', fontsize=16, ha='center', va='bottom')
    plt.text(k_norm[m_LG-1], -17.0, r'$\Gamma$', fontsize=16, ha='center', va='bottom')
    plt.text(k_norm[m_LG+m_GX-1], -17.0, 'X', fontsize=16, ha='center', va='bottom')
    plt.text(k_norm[m_LG+m_GX+m_XK-1], -17.0, 'U,K', fontsize=16, ha='center', va='bottom')
    plt.text(1.0, -17.0, r'$\Gamma$', fontsize=16, ha='center', va='bottom')
    # New letters in between
    plt.text((-1.0 + k_norm[m_LG-1]) / 2, -17.0, r'$\Lambda$', fontsize=16, ha='center', va='bottom')
    plt.text((k_norm[m_LG-1] + k_norm[m_LG+m_GX-1]) / 2, -17.0, r'$\Delta$', fontsize=16, ha='center', va='bottom')
    plt.text((k_norm[m_LG+m_GX+m_XK-1] + 1.0) / 2, -17.0, r'$\Sigma$', fontsize=16, ha='center', va='bottom')
    
    plt.text(-0.925, 15 -0.25, cond, fontsize=14, ha='left', va='top')
...
### Aquí las figuras de las parte de autofunciones
    ...
    ### mejor que las gráficas, lo mejor sería poner los valores de autovectores para cada 



    ## lo mejor va a ser hacer una tabla y reunir todo lo que he sacado de autofunciones
    ...
    if cond in ['C','Si','Ge','Sn','SiC','GaAs','AlP','GaP','AlAs','GaSb','AlSb','InP','InAs','InSb','ZnSe','ZnTe']:
        ...
        labels = [r'$s_a$', r'$s_c$', r'$p_{xa}$', r'$p_{ya}$', r'$p_{za}$', r'$p_{xc}$', r'$p_{yc}$', r'$p_{zc}$', r'$s^*_a$', r'$s^*_c$']  # orbital labels
        linestyles = ['-','-','-.','-.','-.','-.','-.','-.','-','-']
        ...
        for i in range(10):
            plt.plot(k_norm, np.abs(E_eigenvectors_conduccion[:, i]),color=colors[i],linestyle=linestyles[i],linewidth=1, label=labels[i])  # Conduction bands E5–E8
        ...
        plt.axvline(x=k_norm[min_conducion_index-1], color='k', linestyle='-', linewidth=1) # X -> K
        ...
        plt.title('Autovectores de la banda de conducción')
        ...
        labels = [r'$s_a$', r'$s_c$', r'$p_{xa}$', r'$p_{ya}$', r'$p_{za}$', r'$p_{xc}$', r'$p_{yc}$', r'$p_{zc}$', r'$s^*_a$', r'$s^*_c$']  # orbital labels
        labels2 = [r'$s_a2$', r'$s_c2$', r'$p_{xa}2$', r'$p_{ya}2$', r'$p_{za}2$', r'$p_{xc}2$', r'$p_{yc}2$', r'$p_{zc}2$', r'$s^*_a2$', r'$s^*_c2$']  # orbital labels
        labels3 = [r'$s_a3$', r'$s_c3$', r'$p_{xa}3$', r'$p_{ya}3$', r'$p_{za}3$', r'$p_{xc}3$', r'$p_{yc}3$', r'$p_{zc}3$', r'$s^*_a3$', r'$s^*_c3$']  # orbital labels
       
        for i in range(10):#[2,3,4,5,6,7]:#[0,1,8,9]: #loop en los s and s*
            ...
            #plt.plot(k_norm, np.abs(E_eigenvectors_valencia_2[:, i]),color=colors[i],linestyle='--',linewidth=1, label=labels2[i])  # Conduction bands E5–E8
            plt.plot(k_norm, np.abs(E_eigenvectors_valencia_3[:, i]),color=colors[i],linestyle=linestyles[i],linewidth=1, label=labels[i])  # Conduction bands E5–E8
            #plt.plot(k_norm, np.abs(E_eigenvectors_valencia_4[:, i]),color=colors[i],linestyle='-.',linewidth=1, label=labels[i])  # Conduction bands E5–E8
        
        # **Set x and y limits**
        plt.xlim(-1, 0.211)  # Set x-axis limits
        plt.ylim(0, 1)  # Adjust y-axis as needed
        ...
        plt.axvline(x=k_norm[max_valencia_index-1], color='k', linestyle='-', linewidth=1) # X -> K

        # Remove x-axis numbers
        plt.xticks([], [])  # Empty labels
        plt.tick_params(axis='y', which='both', left=False, right=False, labelleft=False)
        # Add custom letters at certain x positions
        y_max = max(np.max(np.abs(E_eigenvectors_valencia[:, i])) for i in range(10))
        y_min = max(np.min(np.abs(E_eigenvectors_valencia[:, i])) for i in range(10))
        plt.text(-1.0, -0.1, 'L', fontsize=12, ha='center', va='bottom')
        plt.text(k_norm[m_LG-1], -0.1, r'$\Gamma$', fontsize=12, ha='center', va='bottom')
        plt.text(k_norm[m_LG+m_GX-1], -0.1, 'X', fontsize=12, ha='center', va='bottom')
        # New letters in between
        ...
        #plt.title('la 4 banda de valencia')
        plt.text(-0.925, 0.8, cond, fontsize=12, ha='left', va='top')
        plt.legend(loc='center left', bbox_to_anchor=(1.02, 0.5), fontsize=9)
        #plt.xlabel(r'vector de onda $k$')
        #plt.gcf().set_size_inches(4.5,4.5)
        plt.tight_layout()
        plt.savefig(f'Figure_autof_valencia_a_{cond}.pdf')
        plt.show()

        for i in range(2, 8): #loop en los p
            plt.plot(k_norm, np.abs(E_eigenvectors_valencia[:, i]),color=colors[i],linestyle='-',linewidth=1, label=labels[i])  # Conduction bands E5–E8
            plt.plot(k_norm, np.abs(E_eigenvectors_valencia_2[:, i]),color=colors[i],linestyle='--',linewidth=1, label=labels[i])  # Conduction bands E5–E8
            plt.plot(k_norm, np.abs(E_eigenvectors_valencia_3[:, i]),color=colors[i],linestyle='-.',linewidth=1, label=labels[i])  # Conduction bands E5–E8
        ...
        if cond == 'GaAs':
            ...
            # Add horizontal lines at the energy values where the min and max energies occur
            plt.axhline(y=min_conducion_energy_SO, color='k', linestyle='--', linewidth=1)
            plt.axhline(y=max_valencia_energy_SO, color='k', linestyle='--', linewidth=1)

            # Shaded area (between the horizontal lines)
            plt.fill_between(k_norm, min_conducion_energy_SO, max_valencia_energy_SO, where=(capa_conducion_SO > min_conducion_energy_SO) & (capa_valencia_SO < max_valencia_energy_SO), 
                         color='gray', alpha=0.3, label='Energy Gap')
            ...
            plt.plot(k_norm, E_SO_eigenvalues[:,0].real,color='k', linewidth=1.5, label='E1')  # Take real part
            plt.plot(k_norm, E_SO_eigenvalues[:,1].real,color='k', linewidth=1.5, label='E2')  # Take real part
            plt.plot(k_norm, E_SO_eigenvalues[:,2].real,color='k', linewidth=1.5, label='E3')  # Take real part
            plt.plot(k_norm, E_SO_eigenvalues[:,3].real,color='k', linewidth=1.5, label='E4')  # Take real part
            plt.plot(k_norm, E_SO_eigenvalues[:,4].real,color='k', linewidth=1.5, label='E5')  # Take real part
            plt.plot(k_norm, E_SO_eigenvalues[:,5].real,color='k', linewidth=1.5, label='E6')  # Take real part
            plt.plot(k_norm, E_SO_eigenvalues[:,6].real,color='k', linewidth=1.5, label='E7')  # Take real part
            plt.plot(k_norm, E_SO_eigenvalues[:,7].real,color='k', linewidth=1.5, label='E8')  # Take real part
            plt.plot(k_norm, E_SO_eigenvalues[:,8].real,color='k', linewidth=1.5, label='E9')  # Take real part
            plt.plot(k_norm, E_SO_eigenvalues[:,9].real,color='k', linewidth=1.5, label='E10')  # Take real part
            plt.plot(k_norm, E_SO_eigenvalues[:,10].real,color='k', linewidth=1.5, label='E11')  # Take real part
            plt.plot(k_norm, E_SO_eigenvalues[:,11].real,color='k', linewidth=1.5, label='E12')  # Take real part
            plt.plot(k_norm, E_SO_eigenvalues[:,12].real,color='k', linewidth=1.5, label='E13')  # Take real part
            plt.plot(k_norm, E_SO_eigenvalues[:,13].real,color='k', linewidth=1.5, label='E14')  # Take real part
            plt.plot(k_norm, E_SO_eigenvalues[:,14].real,color='k', linewidth=1.5, label='E15')  # Take real part
            plt.plot(k_norm, E_SO_eigenvalues[:,15].real,color='k', linewidth=1.5, label='E16')  # Take real part
            plt.plot(k_norm, E_SO_eigenvalues[:,16].real,color='k', linewidth=1.5, label='E17')  # Take real part
            plt.plot(k_norm, E_SO_eigenvalues[:,17].real,color='k', linewidth=1.5, label='E18')  # Take real part
            plt.plot(k_norm, E_SO_eigenvalues[:,18].real,color='k', linewidth=1.5, label='E19')  # Take real part
            plt.plot(k_norm, E_SO_eigenvalues[:,19].real,color='k', linewidth=1.5, label='E20')  # Take real part

            #plt.legend(fontsize=6, loc='upper right', ncol=2)  # Adjust legend size and placement

            # **Set x and y limits**
            plt.xlim(-1, 0.208)  # Set x-axis limits
            plt.ylim(-5, 5)  # Adjust y-axis as needed
            ...
            # Add horizontal lines at the energy values where the min and max energies occur
            plt.axhline(y=min_conducion_energy_SO, color='k', linestyle='--', linewidth=1.5)
            plt.axhline(y=max_valencia_energy_SO, color='k', linestyle='--', linewidth=1.5)
            ...
            plt.xticks([], [])  # Empty labels
            # Add custom letters at certain x positions
            plt.text(-1.0, -5.75, 'L', fontsize=16, ha='center', va='bottom')
            plt.text(k_norm[m_LG-1], -5.75, r'$\Gamma$', fontsize=16, ha='center', va='bottom')
            plt.text(k_norm[m_LG+m_GX-1], -5.75, 'X', fontsize=16, ha='center', va='bottom')
            #plt.text(k_norm[m_LG+m_GX+m_XK-1], -16.5, 'U, K', fontsize=12, ha='center', va='bottom')
            #plt.text(1.0, -16.5, r'$\Gamma$', fontsize=12, ha='center', va='bottom')
            # New letters in between
            #plt.text((-1.0 + k_norm[m_LG-1]) / 2, -16.5, r'$\Lambda$', fontsize=12, ha='center', va='bottom')
            ...
            plt.text(-0.925, 5 -0.15, cond, fontsize=14, ha='left', va='top')
            plt.tick_params(axis='both', labelsize=14)
            #plt.xlabel(r'vector de onda $k$')
            plt.ylabel('E (eV)',fontsize=14)
            plt.gcf().set_size_inches(4.5,4.5)
            ...
            plt.gcf().set_size_inches(4.5,4.5)
            plt.tight_layout()
            plt.savefig(f'Figure_3_{cond}.pdf')
            plt.show()
